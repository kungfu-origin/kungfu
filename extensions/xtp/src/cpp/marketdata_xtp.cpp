...
void MarketDataXTP::on_start() {
  level2_market_data_band_uid_ = request_band("market-data-band");
...
void MarketDataXTP::OnTickByTick(XTPTBT *tbt_data) {
  if (tbt_data->type == XTP_TBT_ENTRUST) {
    Entrust &entrust = get_writer(level2_market_data_band_uid_)->open_data<Entrust>(0);
    from_xtp(*tbt_data, entrust);
    get_writer(level2_market_data_band_uid_)->close_data();
  } else if (tbt_data->type == XTP_TBT_TRADE) {
    Transaction &transaction = get_writer(level2_market_data_band_uid_)->open_data<Transaction>(0);
