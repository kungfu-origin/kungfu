...
@click.option("-s", "--service_vendor", type=str, required=False)
@kfc.pass_context()
def run(
    ctx, mode, category, group, name, low_latency, reference, arguments, service_vendor
):
    ctx.mode = mode
    ...
    ctx.service_vendor = service_vendor
