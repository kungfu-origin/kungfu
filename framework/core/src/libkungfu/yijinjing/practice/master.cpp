...
master::master(location_ptr home, bool low_latency)
  ...
  writers_.emplace(location::UPDATE, io_device->open_writer(location::UPDATE));
  get_writer(location::UPDATE)->mark(start_time_, SessionStart::tag);
}
...
void master::on_exit() {
  ...
  get_writer(location::UPDATE)->mark(now, SessionEnd::tag);
}
...
void master::register_app(const event_ptr &event) {
  ...
  auto update_writer = get_writer(location::UPDATE);
  ...
  require_write_to(event->gen_time(), app_location->uid, location::UPDATE);
...
void master::on_request_read_from_update(const event_ptr &event) {
  const RequestReadFromUpdate &request = event->data<RequestReadFromUpdate>();
  require_read_from_update(event->gen_time(), event->source(), request.source_id, request.from_time);
}
