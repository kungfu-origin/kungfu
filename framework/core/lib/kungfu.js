...
const layout_dir_from_home = function (
  home,
  category,
  group,
  name,
...
const locator = function (home) {
  return {
    ...
    layout_file: function (category, group, name, mode, layout, file_name) {
      return path.join(
        layout_dir_from_home(home, category, group, name, mode, layout),
        file_name + '.' + layout,
      );
    },
    list_page_id: function (category, group, name, mode, dest_id) {
      const suffix = '.journal';
      const dest_dir = layout_dir_from_home(
        home,
        category,
        group,
        name,
      ...
      return pages.map((p) =>
        parseInt(
          p.substr(
            dest_dir.length + 10,
            p.length - dest_dir.length - 10 - suffix.length,
          ),
        ),
      );
    },
