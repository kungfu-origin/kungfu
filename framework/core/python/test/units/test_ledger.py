...
class LedgerClient:
    def __init__(self):
        ...
        self.commander_location = yjj.location(
            lf.enums.mode.LIVE,
        ...
        self.cmd_sock = io_device.connect_socket(
            self.commander_location, yjj.protocol.REQUEST, 10000
        )
    ...
    def _request(self, msg_type, data={}):
        self.cmd_sock.send(
            json.dumps(
                {
                    "msg_type": msg_type,
                    "dest": self.commander_location.uid,
                    "data": data,
                }
            )
        )
