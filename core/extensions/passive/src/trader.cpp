    ...
    namespace wingchun
    {
        namespace passive
        {
            ...
            bool PassiveTrader::insert_order(const yijinjing::event_ptr &event)
            {
                ...
                if (mds.find(std::string(input.instrument_id)) == mds.end())
                {
                ...
                else
                {
                    ...
                    if ((input.price_type == PriceType::Any)||(input.price_type == PriceType::Best)||(input.price_type == PriceType::Best5))
                        rtn_order_from(input, OrderStatus::Filled);
                    else if (input.price_type == PriceType::Limit)
                    {
                        if (input.side == Side::Buy)
                        {
                            if (input.limit_price >= mds[std::string(input.instrument_id)].last_price)
                                rtn_order_from(input, OrderStatus::Filled);
                            else
            ...
            void PassiveTrader::on_md(const msg::data::Quote &quote,uint32_t source)
            {
                ...
                //mds[std::string(quote.instrument_id)] = quote;
            }
