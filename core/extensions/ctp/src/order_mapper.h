...
namespace kungfu::wingchun::ctp {
    ...
    public:
        ...
        void update_sys_id(int front_id, int session_id, const char *order_ref, const char *exchange_id,
        ...
        CtpOrder get_order_info_by_order_ref(int front_id, int session_id, const char *order_ref) {
            CtpOrder result = {};
            SQLite::Statement query(db_,
                                    "SELECT * FROM ctp_order WHERE front_id = ? and session_id = ? and order_ref = ?");
            try {
