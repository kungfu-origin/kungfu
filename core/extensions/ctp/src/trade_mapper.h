...
namespace kungfu::wingchun::ctp {
    ...
    public:
        ...
        void save(uint64_t trade_id, int64_t trade_time, const char *trading_day, const char *exchange_id,
                  const char *ctp_trade_id) {
        ...
        CtpTrade get(const char *trading_day, const char *exchange_id, const char *trade_id) {
            ...
            SQLite::Statement query(db_,
                                    "SELECT * FROM ctp_trade WHERE trading_day = ? AND exchange_id = ? AND ctp_trade_id = ?");
            try {
