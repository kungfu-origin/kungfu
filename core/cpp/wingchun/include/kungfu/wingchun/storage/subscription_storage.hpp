...
namespace kungfu
{
    namespace wingchun
    {
        ...
        public:
            ...
            std::vector<msg::data::Instrument> get_subscriptions()
            {
                ...
                try
                {
                    ...
                    while (query.executeStep())
                    {
                        ...
                        inst.instrument_type = (InstrumentType) query.getColumn(2).getText()[0];
            ...
            void add_subscription(const msg::data::Instrument &instrument)
            {
                try
                {
                    SQLite::Database db(file_name_.c_str(), SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE);
            ...
            void del_subscription(const msg::data::Instrument &instrument)
            {
                try
                {
                    SQLite::Database db(file_name_.c_str(), SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE);
