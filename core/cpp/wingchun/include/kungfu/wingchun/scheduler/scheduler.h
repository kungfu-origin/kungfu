...
namespace kungfu
{
    ...
    public:
        ...
        bool is_recurred() const { return recur_; }
        virtual void exec(int64_t trigger_time) { f_();}
    ...
    public:
        ...
        int64_t get_new_time() const override { return 0; }
    };
    ...
    public:
        ...
        {
            in(now_in_nano() + std::chrono::duration_cast<std::chrono::nanoseconds>(time).count(), std::forward<_Callable>(f), std::forward<_Args>(args)...);
        }
...
//    * * * * *
        ...
        void cron(const std::string &expression, _Callable &&f, _Args &&... args) {
            std::shared_ptr<Task> t = std::make_shared<CronTask>(*this, expression, std::bind(std::forward<_Callable>(f), std::forward<_Args>(args)...));
            auto next_time = t->get_new_time();
            add_task(next_time, std::move(t));
        }
