...
namespace kungfu
{
    namespace wingchun
    {
        typedef std::function<void(const msg::data::Quote &quote)> QuoteCallback;
        typedef std::function<void(const msg::data::Entrust &entrust)> EntrustCallback;
        typedef std::function<void(const msg::data::Transaction &transaction)> TransactionCallback;
        typedef std::function<void(const std::string &source, std::vector<msg::data::Instrument> &, bool is_level2)> SubscribeCallback;
        typedef std::function<void(const std::string &source, const std::string &name, const std::string &client_id)> ReqLoginCallback;
        typedef std::function<void(const msg::data::OrderInput &input)> OrderInputCallback;
        typedef std::function<void(const msg::data::OrderAction &action)> OrderActionCallback;
        typedef std::function<void(const msg::data::Order &order)> OrderCallback;
        typedef std::function<void(const msg::data::Trade &trade)> TradeCallback;
        typedef std::function<void(uint64_t order_id, const std::string &client_id, const std::string &algo_type,
                                   const std::string &algo_order_input)> AlgoOrderInputCallback;
        ...
        typedef std::function<void(msg::data::OrderInput &input)> ManualOrderInputCallback;
        typedef std::function<void(const std::string &account_id, const std::string &client_id,
                                   const std::vector<uint64_t> &order_ids)> ManualOrderActionCallback;
