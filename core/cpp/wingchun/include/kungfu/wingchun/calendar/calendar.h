...
namespace kungfu
{
    namespace wingchun
    {
        ...
        {
        public:
            Calendar(const std::string& calendar_db_file);
            ...
            //@param exchange_id 交易所ID
            //@return 交易所交易时段列表
            std::vector<msg::nanomsg::TradingSession> get_trading_sessions(int64_t start_nano, int64_t end_nano, const std::string &exchange_id);

            //获取当前交易日
            //@return 当前交易日
            int get_current_trading_day();

            //获取下一个交易日
            //@return 下一个交易日
            int get_next_trading_day();
