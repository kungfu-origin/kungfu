...
namespace kungfu
{
    namespace wingchun
    {
        void load_single_pos(SQLite::Statement &query, Position &pos)
        {
            pos = {};
        ...
        void save_single_pos(SQLite::Database &db, const std::string& table, const Position &pos)
        {
        ...
        PositionManager::PositionManager(const std::string& account_id, Calendar& calendar) : account_id_(account_id), calendar_(calendar)
        {}

        Position PositionManager::get_long_pos(const std::string& instrument_id, const std::string& exchange_id) const
        {
        ...
        Position PositionManager::get_short_pos(const std::string& instrument_id, const std::string& exchange_id) const
        {
            auto key = get_symbol(instrument_id, exchange_id);
        ...
        double PositionManager::get_last_price(const std::string& instrument_id, const std::string& exchange_id) const
        {
            auto key = get_symbol(instrument_id, exchange_id);
            auto iter = long_pos_map_.find(key);
            if (iter != long_pos_map_.end())
        ...
        void PositionManager::on_quote(const Quote *quote)
        {
        ...
        void PositionManager::on_order(const Order *order)
        {
        ...
        void PositionManager::on_positions(const std::vector<Position> &positions)
        {
        ...
        void PositionManager::on_position_details(const std::vector<Position> &details)
        {
            // detail只能全刷, 因为无法判断前后
        ...
        void PositionManager::on_account(const AccountInfo &account)
        {}

        void PositionManager::on_order_input(const OrderInput *input)
        {
            // 不处理开仓
        ...
        void PositionManager::on_switch_day(const std::string &trading_day)
        {
        ...
        void PositionManager::register_pos_callback(PositionCallback cb)
        {
            cbs_.emplace_back(std::move(cb));
        }

        void PositionManager::register_acc_callback(AccountCallback cb)
        {}

        void PositionManager::register_pnl_callback(PnLCallback cb)
        {}

        double PositionManager::choose_price(const Position &pos) const
        {
        ...
        bool PositionManager::recalc_pos_by_price(Position &pos)
        {
        ...
        void PositionManager::callback(const Position &pos) const
        {
        ...
        void PositionManager::init_pos_from_input(const OrderInput *input)
        {
            Position pos = {};
            pos.rcv_time = yijinjing::time::now_in_nano();
        ...
        void PositionManager::init_pos_from_trade(const Trade *trade)
        {
            Position pos = {};
        ...
        void PositionManager::deal_bond_from_buy(const Trade *trade)
        {
            Position pos = {};
            pos.rcv_time = trade->rcv_time;
            pos.update_time = trade->rcv_time;
            ...
            auto expire_date = calendar_.calculate_trading_day(std::atoi(pos.open_date), get_reverse_repurchase_expire_days(trade->instrument_id));
            strcpy(pos.expire_date, std::to_string(expire_date).c_str());

            bond_map_[pos.expire_date].emplace_back(pos);
        }

        void PositionManager::insert_detail_from_trade(const Trade *trade)
        {
            Position pos = {};
            pos.rcv_time = trade->rcv_time;
            pos.update_time = trade->rcv_time;
        ...
        void PositionManager::remove_detail_from_trade(const Trade *trade)
        {
        ...
        void PositionManager::on_order_stock(const Order *order)
        {
        ...
        void PositionManager::on_order_future(const Order *order)
        {
            auto key = get_symbol(order->instrument_id, order->exchange_id);
        ...
        void PositionManager::on_trade_stock(const Trade *trade)
        {
            auto key = get_symbol(trade->instrument_id, trade->exchange_id);
        ...
        void PositionManager::on_trade_future(const Trade *trade)
        {
            auto key = get_symbol(trade->instrument_id, trade->exchange_id);
        ...
        void PositionManager::load_from_db(SQLite::Database &db)
        {
        ...
        void PositionManager::dump_to_db(SQLite::Database &db)
        {
