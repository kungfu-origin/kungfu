...
namespace kungfu
{
    namespace wingchun
    {
        ...
        void TaskScheduler::insert_callback_at(int64_t nano, TSCallback cb, bool async)
        {
            int id = get_id();
        ...
        void TaskScheduler::insert_callback_after(int msec, TSCallback cb, bool async)
        {
            insert_callback_at(nano_ + msec * 1000000, cb, async);
        }

        void TaskScheduler::insert_callback_at_next(const char *time_str, TSCallback cb, bool async)
        {
            std::string cur_time_str = yijinjing::time::strftime(nano_, "%Y%m%d-");
            std::string date_time_str = cur_time_str + time_str;
            int64_t nano = yijinjing::time::strptime(date_time_str.c_str(), "%Y%m%d-%H:%M:%S");
            if (nano <= nano_)
            {
                nano += yijinjing::time_unit::NANOSECONDS_PER_DAY;
            }
