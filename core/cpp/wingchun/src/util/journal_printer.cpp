...
namespace kungfu
{
    ...
    void JournalPrinter::run(std::string start_time, std::string end_time, bool keep)
    {
        ...
        kungfu::yijinjing::io_device desk("test", true);
        ...
        do
        {
            ...
            while (frame.has_data() && end_nano > frame.gen_time())
            {
                ...
                if (toPrint)
                {
                    ...
                    if (to_verify_)
                    {
                        int32_t hash = kungfu::yijinjing::MurmurHash2(reinterpret_cast<void *>(frame.address()), frame.data_length(), kungfu::yijinjing::HASH_SEED);
