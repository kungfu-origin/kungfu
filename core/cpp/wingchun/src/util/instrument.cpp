...
namespace kungfu
{
    namespace wingchun
    {
        FutureInstrumentStorage::FutureInstrumentStorage(const std::string &file_name) : db_(file_name.c_str(),
        ...
        void FutureInstrumentStorage::get_future_instrument(std::pair<std::string, std::string> &key,
                                                            std::map<std::pair<std::string, std::string>, FutureInstrument> &future_instruments)
        {
            try
            {
        ...
        void InstrumentManager::reload_from_db()
        {
            ...
            FutureInstrumentStorage storage_(fmt::format(FUTURE_INSTRUMENT_DB_FILE_FORMAT, get_kungfu_home()));
        ...
        double InstrumentManager::get_future_margin_ratio(const char *instrument_id, const char *exchange_id, Direction dir)
        {
            const auto *ins = get_future_instrument(instrument_id, exchange_id);
            if (nullptr != ins)
            {
