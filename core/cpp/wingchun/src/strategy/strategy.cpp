...
namespace kungfu
{
    namespace wingchun
    {
        ...
        public:
            ...
            StrategyUtil_ptr get_util() const
            { return util_; }
            ...
            bool add_md(const std::string &source_id)
            {
                ...
                auto home = strategy_->get_io_device()->get_home();
                auto md = std::make_shared<yijinjing::data::location>(yijinjing::data::mode::LIVE, yijinjing::data::category::MD, source_id, source_id,
                                                                      home->locator);
                strategy_->observe(md);
            ...
            bool add_account(const std::string &source_id, const std::string &account_id, const double cash_limit)
            {
                ...
                strategy_->event_source_->observe(
                        std::make_shared<yijinjing::data::location>(yijinjing::data::mode::LIVE, yijinjing::data::category::TD, source_id, account_id,
                                                                    home->locator));
            ...
            void init()
            {
                ...
                AlgoOrderStatusCallback algo_order_status_callback = std::bind(&Strategy::impl::process_algo_order_status, this,
                                                                               std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);

                int64_t now = yijinjing::time::now_in_nano();
                strategy_->register_nanotime_callback(yijinjing::time::next_minute_nano(now),
                                                      std::bind(&Strategy::impl::on_1min_timer, this, std::placeholders::_1));
                strategy_->register_nanotime_callback(yijinjing::time::next_day_nano(now),
                                                      std::bind(&Strategy::impl::on_daily_timer, this, std::placeholders::_1));
                ...
                strategy_->register_manual_order_action_callback(
                        std::bind(&Strategy::impl::on_manual_order_action, this, std::placeholders::_1, std::placeholders::_2,
                                  std::placeholders::_3));
            }

            void process_quote(const msg::data::Quote &quote)
            {
            ...
            void process_order(const msg::data::Order &order)
            {
            ...
            void process_trade(const msg::data::Trade &trade)
            {
...
//            strategy_->event_source_->get_socket_reply()->send_json(j);
            ...
            void on_1min_timer(int64_t nano)
            {
                ...
                strategy_->register_nanotime_callback(nano + yijinjing::time_unit::NANOSECONDS_PER_MINUTE,
                                                      std::bind(&Strategy::impl::on_1min_timer, this, std::placeholders::_1));
            }
            ...
            void on_daily_timer(int64_t nano)
            {
                ...
                strategy_->register_nanotime_callback(nano + yijinjing::time_unit::NANOSECONDS_PER_DAY,
                                                      std::bind(&Strategy::impl::on_daily_timer, this, std::placeholders::_1));
            }

            int64_t get_nano() const
            { return strategy_->get_nano(); }
        ...
        int64_t Strategy::get_nano() const
        { return impl_->get_nano(); }

        const msg::data::Quote *const Strategy::get_last_md(const std::string &instrument_id, const std::string &exchange_id) const
        {
            return impl_->get_util()->get_last_md(instrument_id, exchange_id);
        }

        msg::data::Position Strategy::get_position(const std::string &instrument_id, const std::string &exchange_id, Direction direction,
                                        const std::string &account_id) const
        {
            return impl_->get_util()->get_position(instrument_id, exchange_id, direction, account_id);
        }

        msg::data::PortfolioInfo Strategy::get_portfolio_info() const
        {
            return impl_->get_util()->get_portfolio_info();
        }

        msg::data::SubPortfolioInfo Strategy::get_sub_portfolio_info(const std::string &account_id) const
        {
            return impl_->get_util()->get_sub_portfolio_info(account_id);
        }

        void
        Strategy::subscribe(const std::string &source, const std::vector<std::string> &instruments, const std::string &exchange_id, bool is_level2)
        {
        ...
        uint64_t
        Strategy::insert_market_order(const std::string &instrument_id, const std::string &exchange_id, const std::string &account_id, int64_t volume,
        ...
        uint64_t Strategy::try_frozen(const std::string &account_id, const msg::nanomsg::AssetsFrozen &frozen)
        {
            //TODO
            return 0;
        }

        bool Strategy::try_frozen(uint64_t op_id, const std::string &account_id, const msg::nanomsg::AssetsFrozen &frozen)
        {
            //TODO
            return false;
        }
        ...
        uint64_t Strategy::try_modify_position(const std::string &account_id, const msg::nanomsg::PositionModify &pos_modify)
        {
            //TODO
            return 0;
        }
