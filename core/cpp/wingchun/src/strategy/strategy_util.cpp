...
#define DUMP_1M_SNAPSHOT(name, pnl) storage::SnapshotStorage(\
    ...
    void StrategyUtil::init(kungfu::yijinjing::event_source_ptr event_source)
    {
    ...
    void StrategyUtil::subscribe(const std::string &source, const std::vector<std::string> &instruments, const std::string &exchange_id, bool is_level2)
    {
    ...
    uint64_t StrategyUtil::insert_order(OrderInput& order_input)
    {
        ...
        writer_->write(0, (int16_t)MsgType::Order, 0, &order_input);
        return uid;
    }
    ...
    uint64_t StrategyUtil::cancel_order(uint64_t order_id)
    {
        ...
        writer_->write(0, (int16_t)MsgType::OrderAction, 0, &action);
...
//        writer_->write_frame_slow(js.c_str(), js.length() + 1, -1, (int)MsgType::AlgoOrderInput, true, -1 );
        return uid;
    }
...
//        writer_->write_frame_slow(js.c_str(), js.length() + 1, -1, (int)MsgType::AlgoOrderAction, true, -1);
    ...
    void StrategyUtil::init_portfolio_manager()
    {
        ...
        if (last_update > 0)
        {
            ...
            SPDLOG_INFO("forward portfolio manager from {}|{} to {}|{}", last_update,
                        ...
                        kungfu::yijinjing::time::strftime(portfolio_manager_->get_last_update(), "%Y%m%d-%H:%M:%S"));
        }
