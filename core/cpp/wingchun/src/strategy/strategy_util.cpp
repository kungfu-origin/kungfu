...
#define DUMP_1M_SNAPSHOT(name, pnl) storage::SnapshotStorage(\
    STRATEGY_SNAPSHOT_DB_FILE(name), PORTFOLIO_ONE_MIN_SNAPSHOT_TABLE_NAME, false, false).insert(pnl)
                ...
                name_(name), has_stock_account_(false), has_future_account_(false)
        {
            ...
            int worker_id = UidWorkerStorage::get_instance(fmt::format(UID_WORKER_DB_FILE_FORMAT, get_kungfu_home()))->get_uid_worker_id(name);
            if (worker_id <= 0)
            {
                UidWorkerStorage::get_instance(fmt::format(UID_WORKER_DB_FILE_FORMAT, get_kungfu_home()))->add_uid_worker(name);
                worker_id = UidWorkerStorage::get_instance(fmt::format(UID_WORKER_DB_FILE_FORMAT, get_kungfu_home()))->get_uid_worker_id(name);
            }
        ...
        void StrategyUtil::init(yijinjing::event_source_ptr event_source)
        {
        ...
        {
            msg::data::SubPortfolioInfo info = {};
            ...
            if (rsp.state != GatewayState::Ready)
            {
                SPDLOG_WARN("trade gateway {}/{} is not ready yet, current state: {}.", source_id, account_id, (int) rsp.state);
                return false;
            } else
            {
                return true;
        ...
        void StrategyUtil::on_quote(const msg::data::Quote &quote)
        {
        ...
        {
            on_quote(*(const msg::data::Quote *) quote);
        }

        void StrategyUtil::on_order(const msg::data::Order &order)
        {
        ...
        {
            on_order(*(const msg::data::Order *) order);
        }

        void StrategyUtil::on_trade(const msg::data::Trade &trade)
        {
        ...
        {
            on_trade(*(const msg::data::Trade *) trade);
        }
        ...
        void StrategyUtil::subscribe(const std::string &source, const std::vector<std::string> &instruments, const std::string &exchange_id,
                                     bool is_level2)
        {
            std::vector<msg::data::Instrument> inst_vec;
            ...
            gateway::subscribe(event_source_->get_io_device(), source, inst_vec, is_level2, this->name_);
        ...
        uint64_t StrategyUtil::insert_order(msg::data::OrderInput & order_input)
        {
            ...
            writer_->write(0, (int16_t) msg::type::Order, &order_input);
            return uid;
        }

        uint64_t StrategyUtil::cancel_order(uint64_t order_id)
        {
            msg::data::OrderAction action = {};
            ...
            writer_->write(0, (int16_t) msg::type::OrderAction, &action);
        ...
        {
            uint64_t uid = next_id();
            msg::nanomsg::AlgoOrderInput input = {};
...
//        writer_->write_frame_slow(js.c_str(), js.length() + 1, -1, (int)msg::type::AlgoOrderInput, true, -1 );
            return uid;
        }
        ...
        uint64_t StrategyUtil::modify_algo_order(uint64_t order_id, const std::string &cmd)
        {
            ...
            msg::nanomsg::AlgoOrderAction action = {};
            action.order_action_id = uid;
            action.order_id = order_id;
            action.action = cmd;

            std::string js = msg::nanomsg::to_string(action);

//        writer_->write_frame_slow(js.c_str(), js.length() + 1, -1, (int)msg::type::AlgoOrderAction, true, -1);

            return uid;
        }

        const msg::data::Quote *const StrategyUtil::get_last_md(const std::string &instrument_id, const std::string &exchange_id) const
        {
            auto iter = quote_map_.find(get_symbol(instrument_id, exchange_id));
            if (iter == quote_map_.end())
            {
        ...
        msg::data::Position StrategyUtil::get_position(const std::string &instrument_id, const std::string &exchange_id, Direction direction,
        ...
        msg::data::PortfolioInfo StrategyUtil::get_portfolio_info() const
        {
            return portfolio_manager_->get_portfolio();
        }

        msg::data::SubPortfolioInfo StrategyUtil::get_sub_portfolio_info(const std::string &account_id) const
        {
            return portfolio_manager_->get_sub_portfolio(account_id.c_str());
        }

        std::vector<uint64_t> StrategyUtil::get_pending_orders(const std::string &account_id) const
        {
        ...
        void StrategyUtil::on_push_by_min()
        {
            ...
            if (is_open)
            {
                ...
                publisher_->publish_portfolio_info(pnl, msg::type::PortfolioByMin);
        ...
        {
            auto pnl = portfolio_manager_->get_portfolio();
            pnl.update_time = (int64_t) std::round((double) yijinjing::time::now_in_nano() / 1000000000) * 1000000000;
        ...
        std::vector<msg::data::SubPortfolioInfo> StrategyUtil::get_accounts()
        {
        ...
        void StrategyUtil::init_portfolio_manager()
        {
            ...
            portfolio_manager_->register_pnl_callback(
                    std::bind(&NNPublisher::publish_portfolio_info, publisher_.get(), std::placeholders::_1, msg::type::Portfolio));
            ...
            if (last_update > 0)
            {
                ...
                {
                    reader->join(
                            std::make_shared<yijinjing::data::location>(yijinjing::data::mode::LIVE, yijinjing::data::category::MD, source,
                                                                        source, io->get_home()->locator), 0, last_update);
                }
                for (const auto &account: get_accounts())
                {
                    reader->join(std::make_shared<yijinjing::data::location>(yijinjing::data::mode::LIVE, yijinjing::data::category::TD,
                                                                             account.source_id, account.account_id, io->get_home()->locator), 0,
                                 last_update);
                }
                ...
                SPDLOG_INFO("forward portfolio manager from {}|{} to {}|{}", last_update,
                            yijinjing::time::strftime(last_update, "%Y%m%d-%H:%M:%S"), portfolio_manager_->get_last_update(),
                            yijinjing::time::strftime(portfolio_manager_->get_last_update(), "%Y%m%d-%H:%M:%S"));
            }
