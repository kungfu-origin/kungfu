...
namespace kungfu
{
    namespace wingchun
    {
        namespace storage
        {
            ...
            CommissionStorage::CommissionStorage(const std::string& account_id, const std::string& file_path) :
                    account_id_(account_id),
                    db_(file_path), hook_handle_(nullptr)
            {
                try
                {
            ...
            void CommissionStorage::load(msg::data::InstrumentCommissionRate &stock_rate,
                                         std::map<std::string, msg::data::InstrumentCommissionRate> &future_rates) const
            {
                stock_rate = {};
                future_rates.clear();
                future_rates[""] = {};
                try
                {
                    ...
                    while (query.executeStep())
                    {
                        ...
                        {
                            memcpy(&stock_rate, &rate, sizeof(msg::data::InstrumentCommissionRate));
                        } else
                        {
            ...
            void CommissionStorage::save(const msg::data::InstrumentCommissionRate &rate) const
            {
                try
                {
            ...
            void CommissionStorage::save(const std::vector<msg::data::InstrumentCommissionRate> &rates) const
            {
                try
                {
                    SQLite::Database db(db_.c_str(), SQLite::OPEN_READWRITE);
            ...
            void CommissionStorage::save(const msg::data::InstrumentCommissionRate &stock_rate,
                                         const std::map<std::string, msg::data::InstrumentCommissionRate> &future_rates) const
            {
                try
                {
                    SQLite::Database db(db_.c_str(), SQLite::OPEN_READWRITE);
            ...
            void CommissionStorage::single_replace(SQLite::Database &db, const msg::data::InstrumentCommissionRate &rate) const
            {
