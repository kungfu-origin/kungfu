...
void Client::subscribe_all(const location_ptr &md_location) { enrolled_md_locations_.emplace(md_location->uid, true); }
...
bool Client::is_subscribed(const location_ptr &md_location, const std::string &exchange_id,
                           const std::string &instrument_id) const {
  return is_all_subscribed(md_location) or
         instruments_.find(get_symbol_id(exchange_id.c_str(), instrument_id.c_str())) != instruments_.end();
}
