...
void Ledger::on_start() {
  events_ | is(Register::tag) | $([&](const event_ptr &event) {
    ...
    if (app_location->category == category::STRATEGY) {
      request_read_from_public(event->gen_time(), app_location->uid, get_last_session_end_time());
      request_read_from(event->gen_time(), app_location->uid, get_last_session_end_time());
