...
void Algo::on_start() {
  ...
  events_ | is(AlgoOrderInput::tag) | $([&](const event_ptr &event) { insert_order(event, event->data_as_string()); });

  events_ | is(OrderAction::tag) | $([&](const event_ptr &event) { cancel_order(event, event->data<OrderAction>()); });

  events_ | is(AlgoOrderModify::tag) | $([&](const event_ptr &event) { modify_order(event, event->data_as_string()); });
