...
AlgoContext::AlgoContext(yijinjing::practice::apprentice &app, const rx::connectable_observable<event_ptr> &events)
  ...
  service_location_ =
      yijinjing::data::location::make_shared(mode::LIVE, category::SYSTEM, "service", "algo", home->locator);
}
...
void AlgoContext::react() {
  ...
  events_ | is(AlgoOrderReport::tag) | $([&](const event_ptr &event) {
    try {
...
void AlgoContext::subscribe(const std::string &source, const std::vector<std::string> &instruments,
...
uint64_t AlgoContext::insert_order(const OrderInput &order) {
  ...
  auto account_location =
      location::make_shared(mode::LIVE, category::TD, order.source_id, order.account_id, home->locator);
