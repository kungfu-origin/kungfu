...
KF_DEFINE_PACK_TYPE(page_header, 1, PK(version), PERPETUAL(), (uint32_t, version), (uint32_t, page_header_length),
                    (uint32_t, page_size), (uint32_t, frame_header_length), (uint64_t, last_frame_position));
...
public:
  ...
  [[nodiscard]] int get_page_id() const { return page_id_; }
