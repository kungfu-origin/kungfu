...
public:
  ...
  virtual void on_child_order(AlgoContext_ptr context, const longfist::types::Order &order){};

  virtual void on_child_trade(AlgoContext_ptr context, const longfist::types::Trade &trade){};

  virtual void on_quote(AlgoContext_ptr context, const longfist::types::Quote &quote){};
...
public:
  explicit AlgoContext(yijinjing::practice::apprentice &app, const rx::connectable_observable<event_ptr> &events);
  ...
  bool has_order(uint64_t order_id) const { return orders_.find(order_id) != orders_.end(); }
  ...
  uint64_t insert_order(const longfist::types::OrderInput &order);
