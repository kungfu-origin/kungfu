...
void Watcher::on_ready() {
  events_ | $([&](const event_ptr &event) { longfist::cast_event_invoke(event, update_state); });
}

void Watcher::on_start() {
  events_ | $([&](const event_ptr &event) { longfist::cast_event_invoke(event, update_ledger); });
