...
void RemoteProcess::setSignal(Signal *the_signal) { this->the_signal = the_signal; }

void RemoteProcess::setCtrlRoutine(CtrlRoutine *the_ctrl_routine) { this->the_ctrl_routine = the_ctrl_routine; }
...
void RemoteProcess::open(void) {
  ...
  if (this->handle == NULL) {
    ...
    if (this->handle == NULL) {
      ...
      this->closeProcessToken();
      throw runtime_error(string("EPERM"));
      // throw runtime_error(std::string("remote-process:open:setPrivilege(true):OpenProcess:code:") +
...
void RemoteProcess::startRemoteThread(void) {
  this->remote_thread = CreateRemoteThread(this->handle, NULL, 0, this->the_ctrl_routine->getAddress(),
                                           (LPVOID)this->the_signal->getType(), CREATE_SUSPENDED, NULL);

  if (this->remote_thread == NULL) {
    this->closeHandle();
    throw system_error(error_code(GetLastError(), system_category()),
                       "remote-process:startRemoteThread:CreateRemoteThread");
  }
  ...
  if (WaitForSingleObject(this->remote_thread, INFINITE) != WAIT_OBJECT_0) {
    ...
    this->closeRemoteThread();
    throw system_error(error_code(GetLastError(), system_category()),
                       "remote-process:startRemoteThread:WaitForSingleObject");
  }
  ...
  if (!GetExitCodeThread(this->remote_thread, (LPDWORD)&exit_code)) {
    ...
    this->closeRemoteThread();
    throw system_error(error_code(GetLastError(), system_category()),
                       "remote-process:startRemoteThread:GetExitCodeThread");
  }
