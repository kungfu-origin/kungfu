...
#ifdef _WINDOWS
            inline DWORD print_stack_trace(EXCEPTION_POINTERS *ep, FILE *out = stderr)
            {
                SPDLOG_ERROR("Unexpected exception");
                StackWalker sw;
                sw.ShowCallstack();
                return EXCEPTION_EXECUTE_HANDLER;
            }
#else
            static inline void print_stack_trace(FILE *out = stderr, unsigned int max_frames = 63) {
