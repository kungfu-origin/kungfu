...
namespace kungfu::node
{
    ...
    void Frame::SetFrame(yijinjing::journal::frame_ptr frame)
    {
    ...
    Napi::Value Frame::GenTime(const Napi::CallbackInfo &info)
    {
        return Napi::BigInt::New(info.Env(), frame_->gen_time());
    }
    ...
    Napi::Value Frame::MsgType(const Napi::CallbackInfo &info)
    {
        return Napi::Number::New(info.Env(), frame_->msg_type());
    }

    Napi::Value Frame::Source(const Napi::CallbackInfo &info)
    {
        return Napi::Number::New(info.Env(), frame_->source());
    }

    Napi::Value Frame::Dest(const Napi::CallbackInfo &info)
    {
        return Napi::Number::New(info.Env(), frame_->dest());
    }

    Napi::Value Frame::ToString(const Napi::CallbackInfo &info)
    {
        return Napi::String::New(info.Env(), "Frame.js");
    }
    ...
    Napi::Value Frame::NewInstance(const Napi::Value arg)
    {
    ...
    Reader::Reader(const Napi::CallbackInfo &info) : ObjectWrap(info), reader(true),
                                                     io_device_(reinterpret_cast<IODevice *>(Napi::ObjectWrap<IODevice>::Unwrap(
    ...
    {
        auto category = get_category_by_name(info[0].As<Napi::String>().Utf8Value());
        auto group = info[1].As<Napi::String>().Utf8Value();
        auto name = info[2].As<Napi::String>().Utf8Value();
        auto mode = get_mode_by_name(info[3].As<Napi::String>().Utf8Value());
