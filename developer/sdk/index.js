...
function getProjectName(packageJson) {
    return packageJson.name.replace('@kungfu-trader/', '').replace('-', '_');
}
...
function generateCMakeFiles(projectName, kungfuBuild) {
    ...
    ejs.renderFile(path.join(__dirname, "templates", "kungfu.cmake"), {
    ...
    ejs.renderFile(path.join(__dirname, "templates", "CMakeLists.txt"), {
