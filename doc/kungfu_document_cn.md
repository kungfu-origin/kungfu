...
```
### <h3 id="进入 docker 实例">进入 docker 实例</h3> 
...
如果不使用提供的 Docker 编译环境，需要自行安装如下依赖包：
...
## Compler 获取代码、编译、安装及更新
以下操作均在docker环境中进行，  [进入 docker 实例](#进入 docker 实例)
...
如需更新功夫到最新版，需执行卸载已安装版本，获取代码最新版本，重新编译和安装

## Run 启动


可使用开箱即用的 Docker 镜像，无需编译即可使用功夫开源交易系统，获取方式如下：
...
```
账户的配置信息中 FrontUri 是前置地址，如果是 simnow 仿真账户可以在 simnow 官网查找，推荐180.168.146.187:10001/10011, UserId 和 InvestorId 均为投资者账户，BrokerId 为券商代码，simnow 账户 BrokerId 一般为 9999。simnow投资者账户需要使用客户端登录并修改密码以后方可使用。
...
如下示例中'Pid'项表示现在'paged'内存数据服务正在运行，'MD‘行情服务正在运行，’TD‘交易服务正在运行，还有一个策略名为'demo_test’的策略正在运行。
...
系统日志存放在 /shared/kungfu/log 目录下，详细内容如下：
...
```
上面的示例中-m参数为信息分类标记，常用的信息分类如下表，全部信息分类可查看源代码/longfist/longfist/LFConstants.h
...
### wingchun pos 持仓设置工具
交易系统支持持仓导出为csv文件以及csv文件设置为持仓，可以使用如下命令查看帮助

```
$ wingchun pos -h
```

下面的示例展示的是将band_demo策略的持仓导出为csv和导入的过程，本方法用于辅助用户将一个账户下的持仓分到多个策略下。
...
```
### wingchun report 统计分析工具

具体使用方法和参数含义可以使用如下命令查看帮助：

```
$ wingchun report -h
```

下面的示例展示了查看band_demo下单委托记录：
...
下面的示例展示了查看band_demo的延迟统计：
一个策略如果有多次交易，可以在 wingchun report 命令中使用 -l 参数列出所有交易时段，然后使用 -i 参数指定交易时段。

```
$ wingchun report -n band_demo -a -i 1
